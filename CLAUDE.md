...
- **Phase 1 (완료)**: Indication selector (OAR/GTV), 4-state Completeness (TP/FN/FP/TN), 1-5 anchor tooltip, XLSX/MD export 분기
- **Phase 2A (완료)**: Summary GTV 지표 (Miss rate, FP count)
- **Phase 2C (완료)**: Completeness 실제/추론 분리 입력 (2-column 체크박스, 기본 truth=Y/pred=N) + 모델/평가지 삭제 기능
- **Phase 3b 결정 변경**: 3-6 Specifications sub-metric을 GTV 공통 라벨로 통일 (Target coverage / Non-target exclusion / Boundary accuracy / Smoothness). OAR은 별도 (Over-seg/Under-seg).
- **GTV subtype 시스템 제거 (2026-05-20)**: 초기 설계에선 GBM/LiverMets/Cervix subtype별로 patient metadata field set이 hard-coded되어 있었으나, 새 indication 추가 시마다 코드 수정 필요해 유연성 부족. **subtype + META_FIELDS + Patient Metadata 카드 완전 제거**. OAR/GTV 두 모드만 유지. 미래에 indication별 정보가 필요하면 free-form metadata 또는 cloud-side schema로 확장 권장.
- **Phase 3 잔여 (미착수)**: Reviewer-level metadata extension, Stratified analysis export, Inter-rater agreement 자동 계산
...
- `currentReadOnly` — true이면 모든 쓰기 차단 (legacy 또는 타인 평가 보기). `body.read-only` 클래스가 #appMain 내 모든 input/select/button을 시각·기능적으로 disable
...
Indication 관련 상태 필드:
- `state.indicationCategory` — `'OAR'` (default) | `'GTV'`
- `state.gtvSubtype` — backward compat용으로 필드 유지 (항상 null로 default). 더 이상 UI에 노출 안 됨
- `state.patientMeta[vi]` / `state.testMeta[ti]` — 데이터 보존용 필드 유지 (저장된 데이터 손실 방지). 더 이상 UI에 노출 안 됨
...
핵심 헬퍼:
- `getCriteria()` — OAR vs GTV 분기 (단순한 두 가지 라벨 set)
...
- `getUsabilityAnchor()` — OAR/GTV별 1-5 scale anchor 텍스트
...
- **Phase 2B (per-lesion nested grid)**: 사용자가 lesion 카테고리별 ROI를 직접 만들어 평가하는 현 방식이 충분한지 사용 후 검토 필요
- **Inter-rater agreement 자동 계산 미구현**: 평가자별 reviews 데이터는 이미 격리되어 있어 통계만 추가하면 됨 (Phase 3 잔여)
- **Reviewer metadata (confidence/duration) 미구현**: review 문서에 필드 추가 + UI만 작업 (Phase 3 잔여)
- **모드 전환 시 데이터 의미**: OAR `true`(missing) ↔ GTV `'FN'` 등 자동 매핑 없음. 평가 도중 mode 변경 시 사용자가 셀 재입력 필요
