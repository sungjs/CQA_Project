...
projects/{projectId}                       # 공유 설정
  ...
  ├─ subMetricLabels: string[4] | null     # owner-defined sub-metric 라벨 (3-6)
  ├─ modality, modalityDetail              # informational (Sprint 3)
  ├─ evaluationStage, modelVersion         # informational (Sprint 3)
  ├─ isNegativeCaseProject: boolean        # Sprint 4 — true면 detection default가 N
...
### 2026-05-21 — Sprint 3 + 4: Project metadata + Negative case project

**Sprint 3** — informational metadata (branching logic 아님, framework v3 § 4 원칙 따라 자유 입력):
- ✅ `state.modality` / `state.modalityDetail` / `state.evaluationStage` / `state.modelVersion`
- ✅ 새 collapsible card "📐 모델 메타데이터" (Reviewer Note 다음, Cutoff 앞)
- ✅ 사이드바 model row에 small badges (`meta-badge` 클래스, neg는 빨강 강조)
- ✅ XLSX A. Summary 시트에 "모델 메타데이터" 블록 추가 (Indication / Modality / Stage / Version / Negative)
- ✅ Owner만 편집 가능

**Sprint 4** — Negative case project (Pillar 3):
- ✅ `state.isNegativeCaseProject: boolean` — 빈 detection 셀의 기본 해석 변경
- ✅ `getDetectionDefault(field, ctx)` 동적 함수 — `tumorPresent` default를 ctx에 따라 'Y' (normal) / 'N' (negative) 전환
- ✅ `deriveDetectionState(detection, ctx)`, `setDetection`, `getDetectionBin` 모두 ctx 인식
- ✅ Toggle 시 데이터 있으면 confirm dialog (의미 변경 경고)
- ✅ Summary "GTV 전체 지표" → "🚫 Negative case GTV — Pillar 3 평가"로 제목 변경 + FP rate를 primary chip으로 강조 (negative project일 때)
- ✅ `gtv-summary-box.negative-mode` 클래스 — amber 배경 + 노란 border
- ✅ Smoke test: `tools/test-migration.js` — 30 assertions (이전 22 + Sprint 4 신규 8개) 모두 통과
