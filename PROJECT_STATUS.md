...
projects/{projectId}                       # 공유 설정
  ...
  └─ reviews/{reviewerUid}                 # 평가자별 격리
       ├─ completeness: { vi: { ri: bool } }   # OAR 모드
       ├─ truthAbsent:  { vi: { ri: true } }   # GTV 모드 (default Y, 'N'만 true 저장)
       ├─ predPresent:  { vi: { ri: true } }   # GTV 모드 (default N, 'Y'만 true 저장)
       ├─ usability:    { vi: { ri: '1'..'5' } }
       ├─ variant:      { ti: { ri: '1'..'5'|'❌' } }
       ├─ specs:        { ri: { ci: bool } }
...
### B. GTV Completeness를 2-column으로
- 원래는 TP/FN/FP/TN 4-state dropdown으로 직접 선택 → 사용자 피드백: "직접 고르기 어렵다"
- 변경: **실제(Truth) 체크박스 + 추론(Pred) 체크박스** 2개로 분리
  - 실제는 기본 체크됨 (있음)
  - 추론은 기본 미체크 (없음 = AI가 안 그림 = FN)
  - 사용자가 추론 체크 = AI가 그림 = TP (또는 FP)
- 두 체크박스로부터 TP/FN/FP/TN 자동 파생 → 셀 배경색 + 좌측 strip
- **이유**: 인지 부담 감소. 두 binary 결정이 4-way classification보다 직관적
...
8. **Sub-metric (3-6) flexibility 없음** — 현재 GBM 형태로 고정 (Target/Non-target/Boundary/Smoothness). Cervix 진입 시 Parametrial extension, Liver mets의 Missed count 같은 sub-metric 필요. owner-defined schema 패턴 권장 (cqa_tool_feedback §3 우려 1).
